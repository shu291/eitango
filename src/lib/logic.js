...
/**
 * 苦手に入れる習熟度の上限（isWeak で使う）。**この値以下（≦）** の語だけが苦手になれる。
 *
 * これより上まで覚えている語は、直近で間違えていても通算成績が悪くても苦手に入れない。
 * ちゃんと覚えている語まで苦手に並べると、本当に手を打つべき語が埋もれるうえ、
 * 苦手リスト・苦手克服モードに出る語が多すぎて回しきれなくなる。
 *
 * 【2026-09-12 更新】以前は「直近で間違えた」なら 60 未満（定着より下）まで、
 * 「通算成績が悪い」なら習熟度に関係なく苦手にしていた。それだと復習に出る語が
 * 多すぎたので、どちらの見方でも **30% 以下** に絞った。
 *
 * 判定に使う progress は間違えて**減点されたあと**の値。
 * 例えば 55%（あと一歩）の語を1回落とすと 43% になり、ここでは苦手にならない。
 * 35%（うろ覚え）の語なら 27% まで落ちるので苦手に入る。
 * 覚えていた語も落とし続ければ 30 以下になり、そこで苦手に入る。
 */
export const WEAK_MAX_PROGRESS = 30;

/**
 * 苦手な単語か。単語一覧の「苦手」タブ・ホームの苦手リスト・苦手克服モード・
 * 出題範囲の「苦手」がすべてこの1つの判定を見ている。ここを直せば全部そろう。
 *
 * まず **習熟度が WEAK_MAX_PROGRESS（30%）以下** であることが前提。
 * それより上まで覚えている語は、以下のどちらに当てはまっても苦手にしない。
 *
 * そのうえで、2つの見方を or でつないである。
 *
 ...
 *    下の 2.（通算成績の判定）に戻る。
 *
 * 2. **通算の成績が悪い** … 2回以上出していて、正答率が5割未満、
 *    または3回以上間違えている。1. を正解で抜けても、
 *    通算成績が悪いままの語はここで苦手に残り続ける（習熟度が 30% を超えるまで）。
 */
export const isWeak = (w) => {
  const correct = w.correct || 0;
  const incorrect = w.incorrect || 0;
  const progress = w.progress || 0;
  // 前提: 習熟度 30% 以下の語だけ。それより覚えている語は苦手にしない
  if (progress > WEAK_MAX_PROGRESS) return false;
  // 1. 最後の答えが不正解 → 次に正解して取り返すまで苦手
  if (isJustMissed(w)) return true;
  // 2. 通算成績で見る
  const t = correct + incorrect;
  if (t < 2) return false;
  const rate = correct / t;
  return rate < 0.5 || incorrect >= 3;
};
