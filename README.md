...
- **学習モード** — フラッシュカード / 4 択クイズ / 例文クイズ / マッチング / スピード / 聞き流し（タイピングと逆引きは 2026-09-13 に削除）
...
- **聞き流し** — 単語 → 意味 → 例文を音声で連続再生。覚え具合で絞り込み、語数・順番・例文の有無を選べる（→ [聞き流しモード](#聞き流しモード)）
- **苦手判定・連続学習日数・週次グラフ**
...
発音が鳴るのは次の 2 か所だけです。他の学習モード（クイズなど）には
意図的に置いていません。
...
（音声の `build:audio` と同じ流れ）。10 語ごとに保存するので、途中で止めても次に回すと続きからになります。
できた `exampleMap.json` は**コミットしてください**。

**速さの目安**（Apple シリコンの MacBook Air）。1回の問い合わせで 10 語まとめて作ります（`--batch`、既定 10）。
1 語ずつ聞くと毎回のやりとりの無駄が大きく、3 倍ほど遅くなります。

| 語数 | gemma3:12b | gemma3:4b |
|---|---|---|
| 2,000 語 | 約 30 分 | 約 10 分 |
| 20,000 語 | 約 5 時間 | 約 2 時間 |

長く回すときは `caffeinate -i npm run build:examples -- --from ...` と頭に付けると、Mac がスリープしません。
12b のほうが日本語訳が自然です。まず `--limit 50` で両方試して見比べるのがおすすめです。
2 万語ぶんの `exampleMap.json` はおよそ 3〜4 MB。アプリの起動時に一度読むだけで、保存領域（5 MB 上限）は使いません。
...
例文の中の見出し語を空欄「______」にして出し、入る単語を4択で選びます。入試の語彙問題に一番近い形です。
例文の日本語訳も一緒に出るので、文脈から考えられます。答えると空欄が埋まり、単語が緑の太字で見えます。

- 出題できるのは **例文があり、その中に見出し語が見つかる語だけ**（`clozeOk`）。設定画面の語数もその中で数えます
- 選択肢は4択クイズと同じ作り方（正解 + 単語帳からランダムに3語）。全体で4語以上必要
- 習熟度の伸びは4択より少し大きい（`MODE_MULT.cloze = 1.1`。文脈から選ぶぶん強い証拠として扱う）
- 空欄の場所は [`splitByHeadword`](src/lib/examples.js) が決めます。活用形（-ed / -ing / -s）も見つけます。
  熟語は語ごとに探し、2文字以下の語（up / to）は文中のあちこちに出るので対象にしません

同じ `splitByHeadword` で、フラッシュカードの裏・単語帳・聞き流しの例文では見出し語を **太字（藍）** にしています。

## 聞き流しモード

学習モード一覧の一番下にある **「聞き流し」** です。画面を触らずに、
**単語 → 意味 → 例文** の順で音声が流れ、次の単語へ自動で進みます。
習熟度は動かしません（答えを申告しないので、テストではないため）。

設定できるもの:

| 項目 | 選べるもの |
|---|---|
| 番号で絞る | 単語帳の何番〜何番か。空なら全部 |
| どの単語 | 全て／苦手／今日の復習／未学習／覚え具合の6段階（それぞれの語数つき。番号で絞った中で数える） |
| 並び順 | 覚えていない順／覚えている順／単語帳の順／ランダム |
| 語数 | 全／10／20／50／100 |
| 流す順 | 英語 → 日本語／日本語 → 英語 |
| 例文 | 流す／流さない（例文が無い語は単語と意味だけ） |
| 再生速度 | 0.7×／0.85×／1×／1.2×／1.5×。単語の音声・意味・例文のすべてに掛かる。再生中にも変えられる（いまの語からやり直す） |
| 終わったら | 止まる／最初から繰り返す |

再生中は「前へ／一時停止・再開／次へ」と「終了」だけ。いま読んでいる部分（単語・意味・例文）が藍色になります。

しくみ（[`src/lib/speech.js`](src/lib/speech.js) / [`speech.web.js`](src/lib/speech.web.js)）:

- `sayWord(word)` … 単語。事前生成の音声（Piper）があればそれ、無ければ端末の英語読み上げ。**鳴り終わるまで待てる**
- `sayText(text, lang)` … 意味（`ja-JP`）と例文（`en-US`）。端末の読み上げ。読み終わるまで待てる
- 順に流す側（`runListen`）は「世代番号」を持ち、一時停止・前へ・次へ・画面を離れる、のどれでも番号を進めて古いループを止める。
  `stopSpeaking()` は待っている側も即座に解放する
- 意味の記号（`～` `；` `〔 〕`）は読み上げ前に落とす（`jaForSpeech`）。「から」「セミコロン」と読まれないため

⚠️ iPhone の Safari は、画面が消えたり別のアプリに切り替えると読み上げが止まることがあります。
長く聞くときは画面を点けたままにしてください。Web 版で最初の1回は必ずボタンのタップから始める必要があります
（自動再生の制限。`speech.web.js` の解錠の仕組みを参照）。

単語帳（管理シート）で行を開くと、例文の右にもスピーカーが付きます。例文の音声は端末の読み上げです。
