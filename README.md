...
**何もしなくてもそのまま復習モードに乗ります。** `due` を持たない単語には、
読み込み時に [`backfillSchedule`](src/lib/logic.js) がこれまでの記録から予定を後付けします。

| 元にする値 | 決めるもの |
|---|---|
| 習熟度 | 間隔（95%なら14日、30%なら2日、など） |
| 正答率 | `ef`（正答率100%で2.5、0%で1.3） |

`最終学習日 + 間隔` を `due` にします。予定がすでにある単語と、一度も学習していない
単語には手を触れないので、**何度読み込んでも安全**です。
